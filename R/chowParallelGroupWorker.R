...
#' @keywords superNOVA
#' @export
chowParallelGroupWorker <- function(job,data,instance){
	...
	if(rownames(instance$matA)==rownames(instance$matB)){
		chow_result = superNOVA::chowCorGroup(design_mat=data$design,matA=instance$matA, matB=NULL, compare = data$compare,
		                      corrType=data$corrType)
	}else{
		chow_result = superNOVA::chowCorGroup(design_mat=data$design,matA=instance$matA, matB=instance$matB, compare = data$compare,
		                      corrType=data$corrType)
	}
	ddcor_res = superNOVA::flattenChow(chow_result,method="BH")
	if(data$verbose){
		cat("Completed run\n")
		cat(paste0(Sys.time(),"\n"))
	}
