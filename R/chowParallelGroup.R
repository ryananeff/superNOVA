...
chowParallelGroup <- function(inputMat, design, outputFile, compare=NULL,
	...
	batchtools::addAlgorithm(name="chow",fun=superNOVA::chowParallelGroupWorker,reg=reg)
	...
	for (i in batchtools::findDone()$job.id){ #for now
	   ...
	   if (i==1){
		   ...
		   if(sigOutput){
			   utils::write.table(result[result[,"pValDiff_adj"]<=sigThresh,],file = paste0(outputFile,".signif.txt"),
